...
const refreshMojangStatuses = async function(){
    loggerLanding.log('Rafraîchir les statuts de Mojang..')
    ...
    try {
        ...
        loggerLanding.warn('Impossible de rafraîchir l\'état des services Mojang.')
...
const refreshServerStatus = async function(fade = false){
    ...
    try {
        ...
        loggerLanding.warn('Impossible de rafraîchir l\'état du serveur, supposé hors ligne.')
        loggerLanding.debug(err)
    }
    if(fade){
...
function asyncSystemScan(mcVersion, launchAfter = true){
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Java est nécessaire<br>pour le lancement.',
                            'Une installation x64 valide de Java 8 est requise pour le lancement.<br><br>Veuillez vous référer à notre <a href="https://github.com/dscalzi/HeliosLauncher/wiki/Java-Management#manually-installing-a-valid-version-of-java">Guide de gestion de Java</a> pour savoir comment installer manuellement Java.',
                            'Je comprends',
                            'Retourner'
                        )
                        setOverlayHandler(() => {
            ...
            if(m.result === true){
                ...
                setOverlayContent(
                    'Un problème inattendu :<br>Échec du téléchargement de Java',
                    'Malheureusement, nous avons rencontré un problème lors de l\'installation de Java. Vous devrez installer manuellement une copie. Veuillez consulter notre <a href="https://github.com/Secteur42/AdlC-Launcher/wiki">Guide de dépannage</a> pour plus de détails et d\'instructions.',
                    'Je comprends'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
            ...
            switch(m.data){
                ...
                // Download & extraction complete, remove the loading from the OS progress bar.
                    ...
                    setLaunchDetails('Java est installé !')
    ...
    setLaunchDetails('Vérification des informations du système..')
...
function dlAsync(login = true){
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erreur de lancement', err.message || 'Contactez nous sur github.')
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec s'est terminé avec le code ${code}, en supposant une erreur.`)
        ...
        if(m.context === 'validate'){
            switch(m.data){
                case 'distribution':
                    ...
                    loggerLaunchSuite.log('Index de distribution validé.')
                    setLaunchDetails('Chargement des informations sur la version...')
                    break
                case 'version':
                    setLaunchPercentage(40, 100)
                    loggerLaunchSuite.log('Informations sur la version chargée.')
            ...
            if(m.result.forgeData == null || m.result.versionData == null){
                ...
                showLaunchFailure('Erreur pendant le lancement', 'Veuillez vérifier la console (CTRL + Shift + i) pour plus de détails.')
            ...
            if(login && allGood) {
                ...
                loggerLaunchSuite.log(`Envoi du compte sélectionné (${authUser.displayName}) au ProcessBuilder.`)
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Connecté aux serveurs')
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Dans le menu principal')
                    }
                }
