        ...
        request.get('https://status.mojang.com/check',
            ...
            function(error, response, body){
                ...
                if(error || response.statusCode !== 200){
                    logger.warn('Impossible de récupérer le statut de Mojang.')
                    logger.debug('Erreur lors de la récupération des statuts de Mojang :', error)
...
exports.authenticate = function(username, password, clientToken, requestUser = true, agent = minecraftAgent){
    return new Promise((resolve, reject) => {
        ...
        request.post(authpath + '/authenticate',
            ...
            function(error, response, body){
                if(error){
                    logger.error('Erreur pendant l\'authentification.', error)
                    reject(error)
                } else {
...
exports.invalidate = function(accessToken, clientToken){
    return new Promise((resolve, reject) => {
        request.post(authpath + '/invalidate',
            ...
            function(error, response, body){
                if(error){
                    logger.error('Erreur pendant l\'invalidation.', error)
                    reject(error)
                } else {
...
exports.refresh = function(accessToken, clientToken, requestUser = true){
    return new Promise((resolve, reject) => {
        request.post(authpath + '/refresh',
            ...
            function(error, response, body){
                if(error){
                    logger.error('Erreur pendant le rafraîchissement.', error)
                    reject(error)
                } else {
                    if(response.statusCode === 200){
                        resolve(body)
